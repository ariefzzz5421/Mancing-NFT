...
async function ownerFor(request: Request) {
  ...
  if (!owner) return { error: json({ error: "Sign in with Privy to sync your watchlist." }, 401), owner: null };
...
export async function GET(request: Request) {
  ...
  const { data, error } = await database()
    ...
    .select("chain,slug,name,image_url,contract_address,notes,target_floors,dev_wallets,added_at")
...
export async function POST(request: Request) {
  ...
  const { data, error } = await database()
    ...
    .upsert({
      ...
      added_at: item.addedAt, updated_at: new Date().toISOString(),
